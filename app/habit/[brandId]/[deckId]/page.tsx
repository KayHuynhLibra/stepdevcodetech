      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 pt-28 sm:pt-32">
        ...
        >
          <Link href={`/habit/${brandId}`}>
            <button
              className="mb-4 sm:mb-6 text-gray-400 hover:text-white transition-colors flex items-center gap-2 text-sm sm:text-base min-h-[44px] px-2"
            >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-4">
            ...
            <p className="text-lg sm:text-xl text-gray-400">{brandInfo.nameVi}</p>
          </div>
        ...
        <motion.div
          ...
          className="mb-8"
        >
          <div className="glass rounded-2xl p-3 sm:p-4 max-w-2xl mx-auto">
            <input
              type="text"
              placeholder="Tìm kiếm lá bài..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full px-4 sm:px-6 py-3 sm:py-4 rounded-lg bg-white/5 border border-white/10 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 transition-colors text-base sm:text-lg"
            />
          </div>
        </motion.div>
        ...
        {!showForm && (
          ...
          >
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="px-6 sm:px-8 py-3 sm:py-4 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold text-base sm:text-lg shadow-lg shadow-purple-500/50 min-h-[44px]"
            >
              + Thêm lá bài mới
            </motion.button>
          </motion.div>
        ...
        {showForm && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="glass rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 max-w-3xl mx-auto mt-6 sm:mt-8"
          >
            <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4 sm:mb-6">Thêm lá bài mới vào {deckInfo.nameVi}</h2>
            <TarotCardForm
      ...
      {selectedCard && (
        ...
        >
          <motion.div
            ...
            className="glass rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 max-w-3xl w-full max-h-[90vh] overflow-y-auto mx-4 sm:mx-6"
          >
            <div className="flex items-start justify-between mb-4 sm:mb-6">
              <div className="flex-1 pr-4">
                <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2">{selectedCard.name}</h2>
                <p className="text-purple-400 font-semibold text-lg sm:text-xl">#{selectedCard.number}</p>
                <p className="text-gray-500 text-xs sm:text-sm mt-1">{brandInfo.nameVi} • {deckInfo.nameVi}</p>
              </div>
              <button
                onClick={() => setSelectedCard(null)}
                className="text-gray-400 hover:text-white text-3xl sm:text-4xl flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center"
            ...
            <div className="space-y-4 sm:space-y-6">
              <div>
                <h3 className="text-lg sm:text-xl font-semibold text-white mb-2 sm:mb-3">Ý nghĩa</h3>
                <p className="text-gray-300 text-sm sm:text-base">{selectedCard.meaning}</p>
              </div>

              <div>
                <h3 className="text-lg sm:text-xl font-semibold text-white mb-2 sm:mb-3">Mô tả</h3>
                <p className="text-gray-300 text-sm sm:text-base leading-relaxed">{selectedCard.description}</p>
              </div>
