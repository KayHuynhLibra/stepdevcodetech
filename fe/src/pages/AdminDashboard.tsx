...
export default function AdminDashboard() {
  ...
  /** Mở tab + tự gắn managedGame (Tarot/Arcana vault context). */
  const selectAdminTab = (id: TabId) => {
    if (id === "inter" || id === "traffic") {
      selectManagedGame("tarot");
    } else if (id === "arcana") {
      selectManagedGame("arcana");
    }
    setTab(id);
  };
  ...
  const tabs: { id: TabId; label: string; show: boolean }[] = [
    ...
    },
    {
      id: "traffic",
      label: "Lưu lượng",
      show: canTraffic && !narrowOnly && !onlOnly,
    },
    {
      id: "inter",
      label: TAB_LABELS.inter,
      show: canInter && !narrowOnly && !onlOnly,
    },
    {
      id: "arcana",
      label: TAB_LABELS.arcana,
      show: canArcanaCfg && !narrowOnly && !onlOnly,
    },
  ...
  return (
    <AppShell maxWidth="lg">
      ...
      {tab === "overview" && (
        <>
          <AdminOpsMap
            visibleTabs={tabs.filter((t) => t.show).map((t) => t.id)}
            onGo={(id) => selectAdminTab(id)}
          />
          {canInvites && (
            <section
      ...
      {tab === "feedback" && canInboxOps && (
        ...
        >
          <LazyFeedbackAdminPanel
            onMsg={setMsg}
            onOpenCount={setFeedbackOpenCount}
          />
        </Suspense>
      )}

      {tab === "mess" && canInboxOps && (
        <Suspense
          fallback={
            <p className="mt-4 text-[11px] text-[var(--play-muted)]">
              Đang tải Mess…
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-sm font-bold text-[var(--play-ink)]">
                    ...
                    <span className="text-[10px] font-semibold text-[var(--play-muted)]">
                      ...
                      {room.hasPassword ? " · 🔒" : ""}
                      {room.mediaMode === "live" ? " · LIVE" : " · Voice"}
                    </span>
                  </p>
                  <div className="flex flex-wrap gap-1">
                    <button
                    ...
                    <button
                      ...
                      onClick={() =>
                        void roomAction(
                          "/api/room/set-media-mode",
                          {
                            roomId: room.roomId,
                            mode:
                              room.mediaMode === "live" ? "voice" : "live",
                          },
                          room.mediaMode === "live"
                            ? `Room ${room.roomId} → Voice`
                            : `Room ${room.roomId} → Live`,
                        )
                      }
                    ...
                    >
                      {room.mediaMode === "live" ? "Ép Voice" : "Bật Live"}
                    </button>
                    <button
                      type="button"
                      ...
                      >
                        <div className="min-w-0">
                          <p className="truncate font-semibold text-[var(--play-ink)]">
                            ...
                            {seat.videoOn ? (
                              <span className="ml-1 text-rose-600">CAM</span>
                            ) : null}
                          </p>
                        ...
                        <div className="flex shrink-0 gap-1">
                          ...
                          {seat.videoOn ? (
                            <button
                              ...
                              onClick={() =>
                                void roomAction(
                                  "/api/room/force-video-off",
                                  { targetSocketId: seat.socketId },
                                  `Tắt cam ${seat.name}`,
                                )
                              }
                              className="rounded-full bg-amber-700 px-2 py-0.5 text-[9px] font-bold text-white"
                            >
                              Tắt cam
                            </button>
                          ) : null}
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <label className="block text-[11px] font-bold text-[var(--play-ink)]">
                  ...
                  <span className="mt-0.5 block text-[10px] font-normal text-[var(--play-muted)]">
                    3–32 ký tự: chữ, số, _ hoặc -. User nhập mã này để cộng xu ảo.
                  </span>
                </label>
                <label className="block text-[11px] font-bold text-[var(--play-ink)]">
                  Số xu cộng
