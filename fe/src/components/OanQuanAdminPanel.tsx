...
export function OanQuanAdminPanel({
  main,
  onMsg,
}: {
  main: boolean;
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await api<{
        ...
        rooms: AdminRoom[];
        stakePresets?: number[];
      }>("/api/admin/oan-quan/rooms");
      setRooms(r.rooms || []);
      setPresets(r.stakePresets || []);
    } catch (e) {
      onMsg(e instanceof Error ? e.message : "Lỗi tải phòng Ô ăn quan");
    } finally {
      setBusy(false);
    }
  }, [onMsg]);
  ...
  const closeRoom = async (id: string) => {
    ...
    setBusy(true);
    try {
      await api(`/api/admin/oan-quan/rooms/${id}/close`, { method: "POST" });
      onMsg(`Đã đóng ${id}`);
      await load();
    } catch (e) {
  ...
  return (
    <section className="app-panel mt-3 space-y-3 p-3 sm:p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <p className="play-heading text-sm">Ô ăn quan · Quản trị</p>
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-[var(--play-muted)]">
                  ...
                  {r.seats?.length
                    ? ` · ${r.seats.map((s) => s.displayName).join(", ")}`
                    : ""}
                </p>
              </div>
