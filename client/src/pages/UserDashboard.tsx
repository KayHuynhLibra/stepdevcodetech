  ...
  return (
    <AppShell maxWidth="md">
      ...
      <section id="avatar-picker" className="app-panel mt-4 p-3">
        ...
        <label className="mt-2 flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#d6f0ff]/80 px-3 py-2.5 text-sm font-bold text-[#0f3d6e] ring-1 ring-[#1a8fd4]/40">
          <input
            type="file"
        ...
        <div className="mt-3 grid grid-cols-5 gap-2 sm:grid-cols-9">
          {isCustomAvatar(currentAvatar) && (
            <button
              type="button"
              disabled={savingAvatar}
              className="rounded-full p-0.5 ring-2 ring-[#1a8fd4] ring-offset-2"
              title="Avatar từ máy"
            >
              <img
                src={currentAvatar}
            ...
            return (
              <button
                ...
                className={`rounded-full p-0.5 transition ${
                  selected
                    ? "ring-2 ring-[#1a8fd4] ring-offset-2"
      ...
      <section className="app-panel mt-4 p-3">
        ...
        <form onSubmit={redeemCoupon} className="mt-2 flex gap-2">
          ...
          <button
            ...
            className="shrink-0 rounded-xl bg-[#0f3d6e] px-4 text-xs font-bold text-white disabled:opacity-50"
          >
            {redeeming ? "…" : "Đổi"}
          </button>
        </form>
        ...
        {bets.length > 0 && (
          <ul className="mt-2 max-h-72 space-y-1.5 overflow-y-auto">
            {bets.map((b) => (
              <li
                key={b.id}
                className="flex items-start justify-between gap-2 rounded-lg bg-white/70 px-2.5 py-2 text-xs ring-1 ring-[#0f3d6e]/10"
              >
