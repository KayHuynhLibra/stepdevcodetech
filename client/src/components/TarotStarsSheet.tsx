...
export function TarotStarsSheet({ open, rows, onClose }: TarotStarsSheetProps) {
  return (
    <BottomSheet open={open} title="Sao bài Tarot" onClose={onClose}>
      <p className="mb-3 text-center text-[11px] text-[#d6f0ff]/55">
        ...
        {rows.map((row) => (
          <li
            ...
            className={`flex items-center gap-3 rounded-xl px-3 py-2.5 ring-1 ${
              row.isYou
                ? "bg-[#1a8fd4]/25 ring-[#1a8fd4]/50"
                : "bg-white/5 ring-white/10"
            }`}
          >
            <span className="w-6 text-center font-display text-sm font-bold text-[#d6f0ff] tabular-nums">
              {row.rank}
            </span>
            <img
              src={row.avatar}
              alt=""
              className="h-9 w-9 rounded-full object-cover ring-1 ring-[#1a8fd4]/35"
            />
