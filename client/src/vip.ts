/** VIP nhiều cấp — mirror server/src/vipTiers.ts */

export const VIP_TIER_MAX = 5;
export const VIP_GRANT_FLOOR = 3;

export const VIP_ROUNDS_THRESHOLDS: ReadonlyArray<{
  tier: number;
  rounds: number;
}> = [
  { tier: 1, rounds: 1_000 },
...
export function computeVipTier(u: {
  ...
  if (typeof u.vipTier === "number" && Number.isFinite(u.vipTier)) {
    return Math.max(0, Math.min(VIP_TIER_MAX, Math.floor(u.vipTier)));
  }
