 ...
 *
 * 1) Sao lưu local → be/data/backups/<stamp>/  (trừ khi BACKUP_SKIP_LOCAL=1)
 * 2) Upload be/data/*.json lên bucket: <prefix>/<stampUtc>/
 ...
 *   BACKUP_S3_PREFIX              mặc định sofia-data
 *   BACKUP_KEEP_REMOTE            mặc định 14
 *   BACKUP_SKIP_LOCAL=1           bỏ bước copy local
 *
 * Chạy: npm run backup:offsite
 * Railway Cron (mount cùng volume vào be/data):
 *   node be/scripts/backup-offsite.mjs
 */
import {
  createHash,
  createHmac,
...
async function main() {
  ...
  const cfg = {
    ...
    prefix: env("BACKUP_S3_PREFIX", "sofia-data").replace(/^\/+|\/+$/g, ""),
  };
