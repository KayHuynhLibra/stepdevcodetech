...
export function attachVoiceSocket(io: Server) {
  io.on("connection", (socket: Socket) => {
    ...
    socket.on(
      "voice:video",
      (payload: { videoOn?: boolean; token?: string }, ack?: Ack) => {
        const auth = resolveVoiceUser(payload?.token);
        if (!auth.ok) {
          ack?.({ ok: false, reason: auth.reason });
          return;
        ...
        if (
          !rateLimit(`voice:video:${auth.user.id}`, 20, 60_000) ||
          !rateLimit(`voice:video:sock:${socket.id}`, 20, 60_000)
        ) {
          ack?.({ ok: false, reason: "Đổi camera quá nhanh" });
          return;
        }
        const result = voiceRoomStore.setSelfVideo(
          socket.id,
          !!payload?.videoOn,
        );
        if (!result.ok) {
    ...
    socket.on(
      "voice:setMediaMode",
      (payload: { mode?: string; token?: string }, ack?: Ack) => {
        const auth = resolveVoiceUser(payload?.token);
        if (!auth.ok) {
          ack?.({ ok: false, reason: auth.reason });
          return;
        }
        const result = voiceRoomStore.setMediaMode(
          socket.id,
          payload?.mode,
        ...
        // Báo mọi client trong phòng tắt cam local nếu về voice
        if (result.room.mediaMode === "voice") {
          io.to(`voice:${result.room.roomId}`).emit("voice:mediaMode", {
            mode: "voice",
            room: result.room,
          });
        } else {
        ...
        auditStore.log({
          actorId: auth.user.id,
          actorName: auth.user.username,
          action: "voice_media_mode",
          detail: `room=${result.room.roomId} mode=${result.room.mediaMode}`,
        });
        ack?.({ ok: true, room: result.room });
      },
    );
    ...
    socket.on(
      "voice:forceVideoOff",
      (payload: { targetSocketId?: string; token?: string }, ack?: Ack) => {
        const auth = resolveVoiceUser(payload?.token);
        if (!auth.ok) {
          ack?.({ ok: false, reason: auth.reason });
          return;
        ...
        const asMod = canModerateVoiceRoom(auth.user);
        const result = voiceRoomStore.forceVideoOff(
          socket.id,
          String(payload?.targetSocketId ?? ""),
          asMod,
        );
        ...
        io.to(result.targetSocketId).emit("voice:forceVideoOff", {
          room: result.room,
        });
        emitRoom(io, result.room.roomId, result.room);
        ack?.({ ok: true, room: result.room });
