...
const CLIENT_DIST = join(__dirname, "..", "..", "fe", "dist");
...
app.get("/api/admin/system-security", async (req, res) => {
  ...
  if (!checks.clientDist) {
    issues.push({
      ...
      severity: "critical",
      title: "Thiếu fe/dist",
      detail: "Chạy build FE (`npm run build -w fe`) trước khi serve production.",
    });
  }
...
/** Danh sách người đã cộng xu ảo (tổng theo user — không lộ mã coupon). */
...
/** User đổi mã cộng xu ảo — không trả danh sách coupon. */
...
app.post("/api/room/force-video-off", (req, res) => {
  const me = requireRoomModerator(req, res);
  if (!me) return;
  const targetSocketId = String(req.body?.targetSocketId ?? "").trim();
  if (!targetSocketId) {
  ...
  const result = voiceRoomStore.staffForceVideoOff(targetSocketId);
  if (!result.ok) return res.status(400).json(result);
  io.to(targetSocketId).emit("voice:forceVideoOff", {
    room: result.room,
  });
  broadcastVoiceRoom(io, result.roomId);
  audit(me, "room_force_video_off", {
    detail: `room=${result.roomId} target=${targetSocketId}`,
  });
  res.json({ ok: true, room: result.room });
});

app.post("/api/room/set-media-mode", (req, res) => {
  const me = requireRoomModerator(req, res);
  if (!me) return;
  const roomId = Number(req.body?.roomId);
  const result = voiceRoomStore.staffSetMediaMode(roomId, req.body?.mode);
  if (!result.ok) return res.status(400).json(result);
  if (isRoomId(roomId)) {
    io.to(`voice:${roomId}`).emit("voice:mediaMode", {
      mode: result.room.mediaMode,
      room: result.room,
    });
    broadcastVoiceRoom(io, roomId);
  }
  audit(me, "room_media_mode", {
    detail: `room=${roomId} mode=${result.room.mediaMode}`,
  });
  res.json({ ok: true, room: result.room });
});
...
/** Emergency ops UI — sống khi fe/dist hỏng; đăng ký trước SPA catch-all. */
...
app.get(["/ops", "/ops.html"], (_req, res) => {
  ...
  if (!existsSync(html)) {
    res.status(503).type("text").send("Ops page missing — check be/public/ops.html");
    return;
  }
  res.setHeader("Cache-Control", "no-store");
  res.sendFile(html);
