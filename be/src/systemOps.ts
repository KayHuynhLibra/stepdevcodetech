...
export function createLocalBackup(opts?: {
  ...
  const dir = dataDirPath();
  if (!existsSync(dir)) {
    return { ok: false, reason: "Thiếu thư mục be/data" };
  }
  const id = stamp();
  ...
  return [
    {
      ...
      detail:
        "Mở https://YOUR_DOMAIN/ops — trang HTML tĩnh trên server, không cần fe/dist. Đăng nhập mainadmin → xem health, backup JSON.",
      when: "web-ok",
    },
    {
      id: "admin-system",
    ...
    },
    {
      ...
      detail:
        "Vào project Railway → Logs / Metrics / Variables / Volume. Restart service, kiểm mount volume vào be/data, disk đầy, crash loop.",
      when: "web-down",
    },
    {
      id: "volume",
      ...
      detail:
        "Volume phải mount đúng path chứa be/data. Quên mount = data ephemeral mất sau redeploy.",
      when: "always",
    },
    {
      id: "local-backup",
    ...
    },
    {
      ...
      detail:
        "Dừng service → copy file từ be/data/backups/<stamp>/*.json vào be/data/ → start lại. Kiểm /health ready=true.",
      when: "web-down",
    },
  ];
}
