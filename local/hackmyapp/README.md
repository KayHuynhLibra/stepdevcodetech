...
Folder **`local/hackmyapp/`** trên máy bạn dùng để **học kiểm thử phòng thủ** trên app SOFIA (local và/hoặc domain **bạn tự vận hành**).

## GitHub chỉ giữ file này

Chi tiết lab (checklist, script `safe-check`, mẫu thông báo ISP, nhật ký, phương thức…) **không push** — đã `.gitignore` (`local/hackmyapp/*`, trừ README này).
...
- Threat model nội bộ: `local/cybersecurity/` (gitignore)  
